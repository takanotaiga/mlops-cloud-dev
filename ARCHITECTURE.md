...
READMEではNode 18+ / Yarnと書かれていますが、Dockerfileとlockfileはnpm/Node 25を使っています。ローカル開発手順とコンテナビルド手順に差があります。
...
`/api/db/query` はブラウザから任意SQL文字列を受け取り、サーバー側DB資格情報で実行します。UI内部用途としては便利ですが、外部公開する場合は非常に強い権限をブラウザ操作に渡す設計です。

本番運用では以下のいずれかが必要です。

- 認証/認可を前段で必ずかける。
- API route側で許可クエリを限定する。
- ユーザー操作ごとの専用APIへ分割する。
...
UIは複数データセットを選択できる作りですが、バックエンドは単一データセットのみ許可します。また対象動画も単一のみです。ユーザー体験としては、UI側でこの制約を明示または制限する必要があります。
...
- `mlops-cloud-backend/README.md` は `Dockerfile.cv` / `Dockerfile.mlx` に言及しますが、実ファイルは `Dockerfile.base` / `Dockerfile.gpu` です。
- `mlops-cloud-ui/README.md` はYarn中心ですが、実際の `Dockerfile` と `package-lock.json` はnpm中心です。
- `terminal_ws_protocol.md` は一部fallback説明がコードと完全一致していません。コード上は最終fallbackが `172.17.0.1` です。
