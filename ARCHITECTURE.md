...
- `mlops-cloud-backend` が動画処理、推論、ハードウェアメトリクス、クリーンアップを提供するPythonワーカー群。
...
| `mlops-cloud-ui` | Web UI、Next.js APIによるDB/S3プロキシ、アップロード、データセット管理、アノテーション、推論/学習ジョブ操作、結果閲覧。 | Next.js 16, React 19, Chakra UI, React Query, AWS SDK v3, SurrealDB client, DuckDB WASM, hls.js | ユーザー操作の入口 |
| `mlops-cloud-backend` | DBをポーリングして動画変換、HLS生成、推論、結果アップロード、メトリクス収集、削除処理を実行。 | Python 3.11, uv, SurrealDB client, boto3, FFmpeg, Torch/CUDA, SAMURAI/SAM2/RT-DETR, psutil, pynvml, paramiko | 非同期ワーカー群 |
...
| `model` | 例: `samurai-ulr`, `t260-ulr`。 |
| `modelSource` | `internet` または `trained`。現バックエンドは `internet/samurai-ulr` と `internet/t260-ulr` をサポート。 |
...
sequenceDiagram
  ...
  participant S3 as MinIO/S3
  participant Model as SAMURAI/T260 pipeline
  ...
  IM->>Model: run one-shot-object-detection/samurai-ulr or t260-ulr
...
- `model = samurai-ulr` または `model = t260-ulr`

`t260-ulr` は SAMURAI checkout ではなく、uv で導入した公式 SAM2.1 と RF-DETR を使います。初期実装では RF-DETR の PyTorch FP16/FP32 推論を優先し、TensorRT export は検証後に有効化する前提です。
...
- uv extra `mlx` で SAM2.1 / RF-DETR / Torch 系依存を同期し、SAM2.1 checkpointをdownload
