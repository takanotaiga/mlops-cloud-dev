...
## P0: Webターミナルが本番境界として危険

対象:

- `mlops-cloud-backend/terminal_manager.py:121-174`
- `mlops-cloud/docker-compose.yml:163-170`

現状:

`terminal-manager` はWebSocketで接続を受け、初回 `auth` フレームのSSHユーザー名/パスワードでホストへSSHし、PTYを開きます。composeでは `8765` をホスト公開しています。

リスク:

- Webアプリからホストシェルへ到達できる。
- WebSocket自体にアプリ認証、TLS、rate limit、監査、操作制限がない。
- 誤公開すると、SSH資格情報を知る利用者がホストで任意コマンドを実行できる。
- セッション録画や操作ログがないため、事後追跡が難しい。

短期対応:

- 本番composeでは `terminal-manager` をデフォルト無効化する。
- 使う場合も閉域ネットワーク限定にする。
- `8765` の公開をやめ、必要な環境だけでプロキシ越しに出す。
- UI側にも機能フラグを入れ、無効時は `/terminal` を隠す。

中期対応:

- アプリ認証/認可を必須にする。
- 操作ログを残す。
- 接続先・実行可能ユーザー・作業ディレクトリを制限する。
- SSHパスワード方式ではなく、短命トークンや管理された接続方式へ変える。

完了条件:

- 本番標準構成ではWebターミナルが外部公開されない。
- 有効化時も認証・認可・監査がある。
- E2Eで無効時に接続できないこと、有効時に認証なし接続が拒否されることを確認できる。
...
- `mlops-cloud-backend/terminal_ws_protocol.md`
...
- terminal protocol docのfallback説明がコードと完全一致していない。
...
2. 本番composeで `terminal-manager` を無効化/閉域化。
3. 推論作成UIをバックエンド制約に合わせる。
4. Training導線をdisabled/preview化、またはworker実装を開始。
5. composeのsecret化とDB/S3内部ネットワーク化。
6. 状態値のnormalizeと仕様化。
7. HLS/派生成果物のデータモデル整理。
8. README更新。
