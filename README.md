...
| `mlops-cloud-backend/` | Python ワーカー群。動画処理、推論、メトリクス、クリーンアップ、ターミナルブリッジを担当。 |
...
| ws://localhost:8765 | Terminal manager WebSocket |
...
- `terminal-manager` は WebSocket からホスト SSH へ接続します。本番では無効化、閉域化、認証付きプロキシなどを検討してください。
...
- 2026-04-29 時点の Phase 1 E2E 期待値は `6 passed, 3 skipped` です。
